...
// - mode 0x15       => all six EO cameras stacked into one 1920x1080 HD-SDI frame
...
module KintexTop_EO_IR_Combined_HD_SDI(
    ...
    wire eo_mode_active        = eo_single_mode_active || eo_stack_mode_active;
    ...
    assign IEG0_PCLK  = ir_mode_active ? IR_SEL_PCLK_BUFG : 1'b0;
    ...
    assign IEG1_PCLK  = ir_mode_active ? IR_SEL_PCLK_BUFG : 1'b0;
    ...
    // For IR modes, capture the selected 640x512 frame in the IR camera clock
    // domain and read it out as a centered 1920x1080 BT.1120-style raster in
    // the 74.25 MHz HD clock domain. This assumes CAM0_PCLK is the board HD
    // reference clock already used by the existing SDI path.
    wire        ir_hd_de;
    wire        ir_hd_hsync;
    wire        ir_hd_vsync;
    wire [19:0] ir_hd_dout;

    IR640x512_To_HD1080p_Buffered u_ir_to_hd (
        .rst_n   (nRESET),
...
module IR640x512_To_HD1080p_Buffered(
    ...
    localparam integer FRAME_DEPTH   = (1 << FRAME_ADDR_W);
    localparam integer FRAME_BITS    = FRAME_DEPTH * 8;
...
module EO1920x1080_Decimate3_FrameBuffer #(
    ...
    parameter        CLOCKING_MODE_STR = "independent_clock"
)(
    input  wire                     rst_n,
    input  wire                     wr_clk,
    input  wire                     wr_hsync,
    ...
    always @(posedge rd_clk) begin
        if (!rst_n) begin
            ...
            frame_valid_meta <= frame_valid_wr;
            frame_valid      <= frame_valid_meta;
        end
    end
...
module EO6Stack_To_HD1080p_Buffered(
    ...
    EO1920x1080_Decimate3_FrameBuffer #(.SRC_W(SRC_W), .SRC_H(SRC_H), .FRAME_ADDR_W(FRAME_ADDR_W), .READ_LATENCY(READ_LATENCY), .CLOCKING_MODE_STR("common_clock")) u_eo_fb0 (
